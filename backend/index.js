...
app.post("/api/users/2fa/generate", async (req, res) => {
    try {
        ...
        res.json({ secret: secret.base32, qrCodeUrl: qrCodeUrl });
    } catch (error) {
...
app.post("/api/users/2fa/verify", async (req, res) => {
    try {
        ...
        const verified = speakeasy.totp.verify({ secret: userRows[0].two_factor_secret, encoding: 'base32', token: token });
        if (verified) {
