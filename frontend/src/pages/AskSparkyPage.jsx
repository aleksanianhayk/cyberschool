            ...
            <div className="flex-1 bg-white rounded-lg shadow p-4 overflow-y-auto mb-4 space-y-4">
                {messages.map((msg, index) => (
                    <div key={index} className={`flex gap-3 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                        ...
                        <div className={`p-3 rounded-lg max-w-lg ${msg.sender === 'user' ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-800'}`}>
                                ...
                                <div className="mt-4 space-y-2">
                                    {msg.options.map((option, i) => (
                                        <button 
                                            ...
                                            className="block w-full text-left p-2 bg-white rounded-md border border-green-300 text-green-700 font-semibold hover:bg-green-50"
                                        >
                                            {option.text}
                                        </button>
                                    ))}
