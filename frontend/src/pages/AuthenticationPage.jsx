        ...
        <div className="flex items-center justify-center min-h-screen bg-gray-100 p-4">
            <div className="w-full max-w-md p-8 space-y-6 bg-white rounded-xl shadow-lg">
                <div className="flex border-b">
                    <button onClick={() => setActiveTab('login')} className={`w-1/2 py-4 text-center font-semibold transition-colors duration-300 ${activeTab === 'login' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-gray-500 hover:text-indigo-500'}`}>
                        Մուտք
                    </button>
                    <button onClick={() => setActiveTab('register')} className={`w-1/2 py-4 text-center font-semibold transition-colors duration-300 ${activeTab === 'register' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-gray-500 hover:text-indigo-500'}`}>
                        Գրանցում
                    </button>
                </div>
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white p-8 rounded-lg shadow-2xl w-full max-w-md text-center">
                ...
                <p className="text-gray-600 mb-6">Մուտքագրեք ձեր Authenticator հավելվածի 6-նիշանոց կոդը։</p>
                {error && <p className="p-2 rounded-md mb-4 bg-red-100 text-red-800">{error}</p>}
                <form onSubmit={handleVerify}>
                    <input type="text" value={token} onChange={(e) => setToken(e.target.value)} placeholder="123456" maxLength="6" className="w-full p-2 text-center text-2xl tracking-widest border rounded-md mb-4"/>
                    <button type="submit" className="w-full px-5 py-2 bg-green-600 text-white font-semibold rounded-lg">Հաստատել</button>
                    <button type="button" onClick={onClose} className="mt-4 text-sm text-gray-500 hover:underline">Չեղարկել</button>
                </form>
            </div>
        </div>
    );
...
const RegisterForm = () => {
    const [formData, setFormData] = useState({
        name: '', email: '', password: '', phone: '', gender: 'male', school_name: '', role: 'student', grade: '2' // Default grade
    });
    const [error, setError] = useState('');
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <input name="password" type="password" placeholder="Գաղտնաբառ" onChange={handleChange} required className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" />
            <input name="phone" placeholder="Հեռախոս" onChange={handleChange} required className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" />
