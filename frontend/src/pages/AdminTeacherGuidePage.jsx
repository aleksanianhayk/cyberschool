...
// The problematic import that caused the crash has been removed from this file.
    ...
    <div className="my-4">
      <Editor
        ...
        init={{
          ...
          menubar: false,
          plugins:
            "lists link image media table code help wordcount autoresize",
          toolbar:
            "undo redo | blocks | bold italic underline | alignleft aligncenter alignright | bullist numlist outdent indent | link image media | table | code",
          content_style:
      ...
      <div className="p-2 border-t flex justify-end gap-2">
        <button
          onClick={onCancel}
          className="px-4 py-1 bg-gray-200 font-semibold rounded-md"
        >
          Cancel
        </button>
        <button
          onClick={() => onSave(content)}
          className="px-4 py-1 bg-green-600 text-white font-semibold rounded-md"
        >
          Save Text
        </button>
      </div>
...
const EditableGuideItem = ({
  ...
  const handleSaveTitle = () => {
    onSaveTitle(item.id, item.title);
    ...
    <div className="my-2 rounded-lg bg-white shadow-sm border border-gray-200">
      <div className="p-3 flex justify-between items-center">
          ...
          {item.content_type === "dropdown" ? (
            isEditingTitle ? (
              <input
                ...
                className="font-bold text-xl bg-transparent border-b-2 border-indigo-500 outline-none w-full"
              />
            ) : (
        ...
        <div className="flex gap-2 flex-shrink-0">
          {item.content_type === "dropdown" && !isEditingTitle && (
            <button
      ...
      {item.content_type === "text" && isEditingText && (
        <div className="p-3">
          <RichTextEditor
            initialValue={item.content_body}
      ...
      {item.content_type === "dropdown" && isOpen && (
        <div className="p-3 border-t">
          ...
          <div className="mt-2 ml-4">
            <button
              onClick={() => onAddChild(item.id, "dropdown")}
              className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded mr-2"
            >
              + Add Dropdown
            </button>
            <button
              onClick={() => onAddChild(item.id, "text")}
...
const AdminTeacherGuidePage = () => {
  ...
  const handleDeleteItem = useCallback(async () => {
    ...
    try {
      ...
      setGuideContent((prevContent) =>
        deleteRecursively(prevContent, deleteTarget.id)
      );
    } catch (error) {
  ...
  const handleAddItem = useCallback(
    async (parentId, type) => {
      ...
      try {
        const res = await axios.post(API_URL, newItemData);
  ...
  return (
    <div className="p-10">
      <h1 className="text-3xl font-bold mb-6">
        Խմբագրել Ուղեցույցը Ուսուցիչների համար
      </h1>
      <div className="bg-gray-50 p-8 rounded-xl shadow-inner">
        ...
        <div className="mt-6 border-t pt-4">
          <button
            onClick={() => handleAddItem(null, "dropdown")}
            className="text-sm bg-indigo-600 text-white px-3 py-2 rounded"
          >
            + Ավելացնել գլխավոր բաժին
          </button>
        </div>
      </div>
      {deleteTarget && (
        <ConfirmationModal
          message={`Վստա՞հ եք, որ ուզում եք ջնջել "${
            deleteTarget.title || "Text Block"
          }" բաժինը և դրա ամբողջ պարունակությունը։`}
